...
def check_exact_value(user_val, value_to_check, instance_name):
    global completion
    global user_prev_val
    if user_val == "give up":
        instance_name.print_msg(3)
        if instance_name == a_elastic_msg:
            append_wrong_msg(user_prev_val, value_to_check)
        completion = True
...
a_elastic_msg.wrong = f"Company A's price elasticity was {a.elastic} because its price elasticity was {a.price_elasticity}."
...
# Comparative Price Elasticity of Company A and B

ab_price_elasticity = AnswerMessages()
ab_price_elasticity.correct = f"Correct! The CPE for Companies A and B is {ans_ab.price_elasticity:.2f}"
ab_price_elasticity.wrong = f"The correct CPE for Companies A and B was {ans_ab.price_elasticity:.2f}.\n" \
              f"CPE is the % change in demand for Product B divided by the % change in price for Product A.\n" \
              f"% change in demand is (new sales - old sales) / old sales * 100 or ({b.sales_new} - {b.sales_old}) / {b.sales_old} * 100.\n" \
              f"% change in price is (new price - old price) / old price * 100 or ({a.price_new} - {a.price_old}) / {a.price_old} * 100.\n" \
              f"So the CPE is {b.change_demand}% / {a.change_price}%."

completion = False
while completion == False:
    user_ab_price_elasticity = get_user_input("Enter the Comparitive Price Elasticity for Companies A and B,\n" \ 
                                            "\nrounding to 2 decimal places: ", float)
    check_float_value(user_ab_price_elasticity, ans_ab.price_elasticity, ab_price_elasticity)
...
print()
print("Enter \"subsitute products\" or \"complementary products\")
user_ab_status = get_user_input("Company A and Company B sell ", "string")
check_exact_value() # See how this is changed for STRINGS< REVIWSIONS BELOW QAND ABOVE HERE
