...
class TotalInfo:
    def __init__(self):
        ...
        self.market_growth_rate = ((self.sales_dollars - self.last_year_sales) / self.last_year_sales) * 100
...
class LongQuestion:
    def __init__(self):
        ...
        self.show_middle_top, self.show_middle_middle, self.show_middle_bottom, self.show_right_top, self.show_right_middle, \
             self.show_right_bottom, self.show_market_size = False, False, False, False, False, False, False
        self.middle_top, self.middle_middle, self.middle_bottom, self.right_top, self.right_middle, self.right_bottom = [None] * 6
    ...
    def assign_answer_vals(self):
        if self.question_type == "units":
            # This function does assigns the variables with units related items.
            self.market_size = total.sales_units
        ...
        if self.question_type == "dollars":
            # This  function assigns the problem values and answers with dollars related items.
            self.market_size = total.sales_dollars
    ...
    def get_var(self, show_var, val):
        if show_var == True:
            if isinstance(val, float):
                return f"{val:.2f}" # Rounds dollar values / percentages before presentation.
            else:
                return val # Returns whole numbers as they are.
        else:
            return "??"
    ...
    def print_question(self):
        self.assign_answer_vals()
        self.print_question_header()
        self.print_table_header()
        print(f"Your Company:\t\t{self.get_var(self.show_middle_top, self.middle_top)}\t\t{self.get_var(self.show_right_top, self.right_top)}%")
...
market_size_units.correct = f"Correct. The market size in units is {total.sales_units} units."
market_size_units.wrong = f"The correct market size in units was {total.sales_units} units.\n" \
...
my_ms_units.correct = f"Correct! Your Company's Market Share is {my_answers.ms_units:.3f}% (3 decimal places for reference)."
...
competitor_ms_units.wrong = f"The correct Market Share in units for Company B was {competitor_answers.ms_units:.2f}%.\n" \
              ...
              f"In this case, that is {competitor.sales_units} / {total.sales_units} * 100 to convert to a percent."
...
lead_ms_units.wrong = f"The correct Market Share in units for the Lead Competitor was {lead_answers.ms_units:.2f}%\n" \
              ...
              f"In this case, that is {lead_competition.sales_units} / {total.sales_units} * 100 to convert to a percent."

# Lead Competitor Market Share Question
print()
completion = False
while completion == False:
    user_lead_ms_units = get_user_input("Enter the Lead Competitor's market share in units as a percentage up to 2 decimal places: ", float)
    check_float_value(user_lead_ms_units, lead_answers.ms_units, lead_ms_units)
...
long_question2.question_type = "dollars"
long_question2.show_middle_top, long_question2.show_right_top = True, True
long_question2.show_right_middle, long_question2.show_right_bottom = True, True
long_question2.print_question()

# Company B Sales Dollars Setting Up Answer Messages
competitor_sales_dollars = AnswerMessages()
competitor_sales_dollars.precursor = "$"
...
lead_competition_sales_dollars.precursor = "$"
lead_competition_sales_dollars.correct = f"Correct! The total sales of the Lead Competitor was this year is ${lead_competition.sales_dollars}."
...
completion = False
while completion == False:
    user_lead_sales_dollars = get_user_input("Enter the Lead Competitor's Total Sales to the nearest dollar: $", int)
...
# Update longquestion and print.
long_question2.count += 1
long_question2.show_market_size = True
long_question2.print_question()
print()
