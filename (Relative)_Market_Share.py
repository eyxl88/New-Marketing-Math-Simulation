...
company_class = my_company # Default so that program doesn't break.
ans_class = my_answers # Default so that program doesn't break.
company_name = "" # Default
rel_ms.correct = f"Correct! The Relative Market Share is {ans_class.rel_ms:.3f}% (3 decimal places for reference).\n" 
rel_ms.wrong = f"The correct Relative Market Share was {ans_class.rel_ms:.3f}% rounded up or down as necessary.\n" \
              f"Relative Market Share is {company_name} sales in dollars divided by the lead competitor's sales in dollars.\n" \
              f"In this case, that is {company_class.sales_dollars} / {lead_competition.sales_dollars} * 100 to convert to a percent.\n" \
...
print()
rel_ms.company_class = my_company
rel_ms.ans_class = my_answers
...
while completion == False:
    ...
    check_float_value(user_my_rel_ms, my_answers.rel_ms, rel_ms)

# Competitor's Relative Market Share
print()
rel_ms.company_class = competitor
rel_ms.ans_class = competitor_answers
...
while completion == False:
    ...
    check_float_value(user_competitor_rel_ms, competitor_answers.rel_ms, rel_ms)

# Market Growth Rate Problem Printing:
print()
print(f"Last year's total sales in the industry were: ${total.last_year_sales}")
...
total_sales.wrong = f"The correct total sales was ${total.sales_dollars}.\n" \
            f"This is Your Company's sales {my_company.sales_dollars} + Company B's Sales {competitor.sales_dollars} " \
            f"+ Lead Competitor's Sales {lead_competition.sales_dollars}"

# Total Sales question section.
completion = False
while completion == False:
    user_total_sales = get_user_input("$", float)
...
completion = False
while completion == False:
    user_market_growth = get_user_input("Enter the market growth/shrink rate as a percentage rounded to 2 decimal places: ", float)
    check_float_value(user_market_growth, total.market_growth_rate, market_growth)

# Segment Targeting Problem Printing
print()
...
print(f"Segment A is {segment_a.percent}% of the market.")

# Set up Answer Messages:
ms_sega = AnswerMessages()
ms_sega.correct = f"Correct! The Market Share for Your Company in Segment A is {my_answers.ms_seg:.2f}%."
ms_sega.wrong = f"The correct Market Share for Your Company in Segment A was {my_answers.ms_seg:.3f}% rounded up or down as necessary.\n" \
    f"Market Share within a segment is your sales in dollars divided by total sales in that segment in dollars.\n" \
    f"Total sales in Segment A is Segment A's percent of the market / 100 * total market sales in dollars.\n" \
    f"In this case, that is {segment_a.percent} / 100 * {total.sales_dollars} = {segment_a.dollars:.2f}.\n" \
    f"Then, Market Share is {my_company.sales_dollars:.2f} / {segment_a.dollars:.2f} * 100 to convert to a percent."

# Check the Answers for Your Company's Market Share of Segment A.
print()
completion = False
while completion == False:
    user_my_ms_sega = get_user_input("Enter YOUR company's market share in Segment A as a percentage up to 2 decimal places: ", float)
    check_float_value(user_my_ms_sega, my_answers.ms_seg, ms_sega)

# FINISHED REVISIONS UP UNTIL AREA BELOW ______________________________________________
...
market_growth = ((total_sales_dollars - self.last_year_sales) / self.last_year_sales) * 100
