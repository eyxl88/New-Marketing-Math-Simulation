...
# Set problem values.
unit_vc = round(random.uniform(2, 100), 2)
expected_sales = random.randint(100, 1000)
unit_fc = round(random.uniform(1, 15), 2)
total_fc = round(unit_fc * expected_sales, 2)
unit_cost = round(unit_vc + total_fc / expected_sales, 2)
...
unit_cost_msgs.wrong = f"The correct unit cost was ${unit_cost:.2f}.\n" \
...
while completion == False:
    user_unit_cost = get_user_input("Enter the unit cost in dollars up to 2 decimal places: $", float)
    check_float_value(user_unit_cost, unit_cost, unit_cost_msgs)
